...
class GymV26CompatibilityV0(gymnasium.Env[ObsType, ActType]):
    ...
    def __init__(
        ...
        if GYM_IMPORT_ERROR is not None:
            raise error.DependencyNotInstalled(
                f"{GYM_IMPORT_ERROR} (Hint: You need to install gym with `pip install"
                " gym` to use gym environments"
            )
        ...
        if env is not None:
            ...
            raise MissingArgument(
                "Either env_id or env must be provided to create a legacy gym"
                " environment."
            )
    ...
    def reset(
        ...
        # Isaac and Steven's No Seed(/Options) Hack:
        try:
            return self.gym_env.reset(seed=seed, options=options)
        except TypeError:
            return self.gym_env.reset()
        # There is a chance this may break something, but it seems fine for now.
...
class GymV21CompatibilityV0(gymnasium.Env[ObsType, ActType]):
    ...
    def reset(
        ...
        if options is not None:
            warn(
                "Gym v21 environment do not accept options as a reset parameter,"
                f" options={options}"
            )
...
def _convert_space(space: gym.Space) -> gymnasium.Space:
    ...
    if isinstance(space, gym.spaces.Discrete):
        ...
        raise NotImplementedError(
            f"Cannot convert space of type {space}. Please upgrade your code to"
            " gymnasium."
        )
